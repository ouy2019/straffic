...
<div id="page">
  ...
  <div class="detail">
    ...
    <van-cell-group :border="false">
      ...
      <van-cell :border="false" title="本次报销(元)" :value="dataObject.paymentReport.abroadContractsTotal | num" size="small" class="text_l"  />
      <van-cell :border="false" title="报销总额(元)" :value="dataObject.reimbursement.amount | num" size="small" class="text_l"  />
    </van-cell-group>
  </div>
  <!-- 因公出国（境） -->
  <div class="direct" v-if="!dataObject.abroad.length==''">
    <div class="line"></div>
    <div class="detail">
      <div class="title"><img :src="shixiangIcon" alt="" srcset="" class="sxIcon" />出国信息</div>
      <div v-for="(item,index) in dataObject.abroad" :key="index">
        <div v-if="index >= 1" class="spaceKey"></div>
        <van-cell-group :border="false">
          <van-cell :border="false" title="国家" :value="item.country.area" size="small" class="text_l" />
          <van-cell :border="false" title="出访地区" :value="item.regional.area" size="small" class="text_l" />
          ...
          <van-collapse v-model="AcTab">
            <van-collapse-item title="支出详情" name="1">
              <el-table :data="item.details" style="width: 100%" >
                ...
                <el-table-column prop="reimbursementAmount" label="申请金额"></el-table-column>
              </el-table>
            </van-collapse-item>
          </van-collapse>
        </div>
...
export default {
    ...
    methods:{
      ...
      gofilesPdf(filesName,filesUrl){//在线预览文档
        this.$toast.loading({ message: '加载中...',forbidClick: true,});
        this.$native.loadpage(filesUrl,filesName);
      }
    },
