  ...
  <div class="shixiang" v-show="index == 0">
    ...
    <div class="detail">
      ...
      <van-cell-group v-if="dataObject.unit" :border="false">
        ...
        <van-cell :border="false" title="支出事由" :value="dataObject.applyPhoneNumber" size="small" class="text_l" />
        <van-cell :border="false" title="事前业务标题" :value="dataObject.paymentReport.title" size="small" class="text_l" />
        <van-cell :border="false" title="本次报销(元)" :value="dataObject.paymentReport.meetingContractsTotal.toFixed(2)" size="small" class="text_l"  />
        <van-cell :border="false" title="报销总额(元)" :value="dataObject.driver.toFixed(2)" size="small" class="text_l" />
      </van-cell-group>
        <!-- 值班,带班补助 -->
          ...
          <!-- <div v-for="(item,index) in dataObject.indices" :key="index">
            <van-cell-group :border="false">
              <van-cell :border="false"
                title="项目名称"
                :value="item.reportIndex.index.projectName"
                size="small"
                class="text_l"
              />
              <van-cell :border="false"
                title="指标余额"
                :value="item.reportIndex.index.allocationAmount.toFixed(2)"
                size="small"
                class="text_l"
              />
              <van-cell :border="false"
                title="申请金额"
                :value="item.applyAmount.toFixed(2)"
                size="small"
                class="text_l"
              />
              <van-cell :border="false"
                title="支出明细"
                :value="item.reportIndex.index.largeProjectName"
                size="small"
                class="text_l"
              />
            </van-cell-group>
