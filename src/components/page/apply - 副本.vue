  ...
  <div id="page">
    <Nav></Nav>
      ...
      <van-list
        ...
        <div class="listBox">
          <div class="content" v-for="(item, index) in list" :key="index" @click="goDetail(item.title,item.id,item.expenseType)" >
...
export default {
  ...
  mounted() {},
  methods: {
    goDetail(title,id,expenseType) {
      //  this.$router.push({path:"/cost",query:{id:e.id,type:e.taskType,title:title}});
      this.$native.forward({
        path: "/cost",query: { id: id, title: title,type:expenseType },
      });
    },
    init() {  ///app/index/reports?page=0&size=10
      this.$axios.get(apiAddress + "/app/index/reports?", {
